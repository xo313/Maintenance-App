...
async function openApp(): Promise<Session> {
  ...
  const app = await electron.launch({
    executablePath: process.env.CI ? undefined : path.join(projectRoot, 'release_final2', 'win-unpacked', 'Maintenance App Beta.exe'),
...
test('UAT-03: navigates across all main pages', async () => { const s = await openApp(); try { for (const name of ['العمليات والصيانة', 'العملاء', 'الخزينة والسحوبات', 'الإعدادات']) await go(s.page, name); await expect(s.page.getByText('الإعدادات', { exact: true }).first()).toBeVisible(); } finally { await closeApp(s); } });
...
test('UAT-31: opens withdrawals page', async () => { const s = await openApp(); try { await go(s.page, 'الخزينة والسحوبات'); await expect(s.page.getByText('الخزينة والسحوبات', { exact: true }).first()).toBeVisible(); } finally { await closeApp(s); } });
test('UAT-32: adds a shop withdrawal', async () => { const s = await openApp(); try { await go(s.page, 'الخزينة والسحوبات'); await s.page.locator('.form-group').filter({ hasText: 'المبلغ' }).locator('input, select, textarea').first().fill('1000'); await s.page.locator('.form-group').filter({ hasText: 'البيان / الوصف' }).locator('input, select, textarea').first().fill('UAT shop expense'); await s.page.getByRole('button', { name: 'إضافة سحب', exact: true }).click(); await expect(s.page.getByText('UAT shop expense', { exact: true })).toBeVisible(); } finally { await closeApp(s); } });
...
test('UAT-41: restore backup restarts app and preserves data integrity (Cash Math)', async () => {
  // We run our dedicated forensic QA script which simulates the exact scenario
  // including Backup, Restore, and strict math validation of the Cash Ledger.
  // This avoids the Playwright disconnect issue on Electron app.relaunch().
  const { execSync } = require('child_process');
  try {
    const output = execSync('npx tsx test_qa_restore_math.ts', { encoding: 'utf-8', cwd: process.cwd() });
    expect(output).toContain('SUCCESS! ALL TESTS PASSED.');
  } catch (err: any) {
    throw new Error('UAT-41 QA Script Failed: ' + (err.stdout || err.message));
  }
});
