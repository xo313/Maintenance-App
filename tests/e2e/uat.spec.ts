...
test('UAT-05: Arabic text renders without mojibake', async () => { const s = await openApp(); try { await expect(s.page.getByText('لوحة التحكم', { exact: true }).first()).toBeVisible(); await expect(s.page.getByText('العمليات والصيانة', { exact: true }).first()).toBeVisible(); } finally { await closeApp(s); } });
test('UAT-06: operations search controls are available after prerequisite setup', async () => { const s = await openApp(); try { await ensureTechnician(s.page); await expect(s.page.getByLabel('بحث نصي (رقم، اسم، هاتف، جهاز)', { exact: true })).toBeVisible(); } finally { await closeApp(s); } });
...
test('UAT-15: payment input has a max equal to sale price', async () => { const s = await openApp(); try { await ensureTechnician(s.page); await go(s.page, 'العمليات والصيانة'); await s.page.getByRole('button', { name: 'عملية جديدة', exact: true }).click(); const price = s.page.getByLabel('المبيعات (السعر)', { exact: true }); const paid = s.page.getByLabel(/المبلغ الواصل من الزبون/); await price.fill('50000'); await expect(paid).toHaveAttribute('max', '50000'); } finally { await closeApp(s); } });
...
test('UAT-22: dashboard shows the delivered operation in summary', async () => { const s = await openApp(); try { await ensureTechnician(s.page); await addOperation(s.page, { customer: 'UAT Op 22', price: '100000', cost: '30000', paid: '100000', status: 'delivered' }); await go(s.page, 'لوحة التحكم'); await expect(s.page.getByText('لوحة التحكم', { exact: true }).first()).toBeVisible(); await expect(s.page.getByText(/100000|100,000|1,00000/).first()).toBeVisible(); } finally { await closeApp(s); } });
...
test('UAT-25: text search finds an operation by customer', async () => { const s = await openApp(); try { await ensureTechnician(s.page); await addOperation(s.page, { customer: 'UAT Search 25', price: '10000', cost: '1000', paid: '10000' }); await s.page.getByLabel('بحث نصي (رقم، اسم، هاتف، جهاز)', { exact: true }).fill('UAT Search 25'); await expect(s.page.getByText('UAT Search 25', { exact: true })).toBeVisible(); } finally { await closeApp(s); } });
test('UAT-26: cash filter returns only cash operation', async () => { const s = await openApp(); try { await ensureTechnician(s.page); await addOperation(s.page, { customer: 'UAT Cash 26', price: '10000', paid: '10000' }); await s.page.getByLabel('حالة الدفع', { exact: true }).selectOption('cash'); await expect(s.page.getByText('UAT Cash 26', { exact: true })).toBeVisible(); } finally { await closeApp(s); } });
test('UAT-27: partial filter returns partial operation', async () => { const s = await openApp(); try { await ensureTechnician(s.page); await addOperation(s.page, { customer: 'UAT Partial 27', price: '10000', paid: '3000' }); await s.page.getByLabel('حالة الدفع', { exact: true }).selectOption('partial'); await expect(s.page.getByText('UAT Partial 27', { exact: true })).toBeVisible(); } finally { await closeApp(s); } });
test('UAT-28: debt filter returns debt operation', async () => { const s = await openApp(); try { await ensureTechnician(s.page); await addOperation(s.page, { customer: 'UAT Debt 28', price: '10000', paid: '0' }); await s.page.getByLabel('حالة الدفع', { exact: true }).selectOption('debt'); await expect(s.page.getByText('UAT Debt 28', { exact: true })).toBeVisible(); } finally { await closeApp(s); } });
test('UAT-29: completed status filter returns completed operation', async () => { const s = await openApp(); try { await ensureTechnician(s.page); await addOperation(s.page, { customer: 'UAT Status 29', price: '10000', paid: '10000', status: 'completed' }); await s.page.getByLabel('حالة الجهاز', { exact: true }).selectOption('completed'); await expect(s.page.getByText('UAT Status 29', { exact: true })).toBeVisible(); } finally { await closeApp(s); } });
