...
var dbPath = !electron.app.isPackaged ? path.default.join(electron.app.getAppPath(), "database.json") : path.default.join(electron.app.getPath("userData"), "database.json");
var SimpleDB = class {
	data = {
		settings: {
			id: 1,
	...
	load() {
		if (fs.default.existsSync(dbPath)) {
			this.data = JSON.parse(fs.default.readFileSync(dbPath, "utf8"));
			if (!this.data.months) this.data.months = [];
			if (this.data.months.length === 0) this.data.months.push({
				id: 1,
				month_name: (/* @__PURE__ */ new Date()).toLocaleDateString("ar-EG", {
					month: "long",
					year: "numeric"
				}),
			...
			if (!this.data.settings.whatsapp_template) this.data.settings.whatsapp_template = "السلام عليكم [اسم_الزبون] 👋\nنود إعلامك بأن جهازك ([اسم_الجهاز]) قد تمت صيانته وهو جاهز للاستلام.\nالمبلغ المطلوب: [المبلغ] دينار.\nشكراً لاختيارك مركزنا! 🛠️✨";
			if (!this.data.settings.theme) this.data.settings.theme = "dark";
			const currentMonthId = this.getCurrentMonth().id;
			if (this.data.operations) this.data.operations.forEach((op) => {
				if (!op.payment_status) op.payment_status = "cash";
				if (!op.month_id) op.month_id = currentMonthId;
				if (op.faults === void 0 && typeof op.device === "string" && op.device.includes(" - ")) {
					const parts = op.device.split(" - ");
					op.device = parts[0].trim();
					op.faults = [parts.slice(1).join(" - ").trim()];
				} else if (op.faults === void 0) op.faults = [];
				if (!op.status) op.status = "delivered";
			});
			if (this.data.withdrawals) this.data.withdrawals.forEach((w) => {
				if (!w.month_id) w.month_id = currentMonthId;
			});
			if (this.data.technicians) this.data.technicians.forEach((t) => {
				if (t.start_balance === void 0) t.start_balance = 0;
				if (t.is_active === void 0) t.is_active = true;
			});
			...
			const seedPath = path.default.join(electron.app.getAppPath(), "default_seed.json");
			if (fs.default.existsSync(seedPath)) try {
				const seedData = JSON.parse(fs.default.readFileSync(seedPath, "utf8"));
				if (seedData.common_devices) {
					const existingDevices = new Set(this.data.common_devices.map((d) => d.toLowerCase()));
					for (const d of seedData.common_devices) if (!existingDevices.has(d.toLowerCase())) {
						this.data.common_devices.push(d);
						existingDevices.add(d.toLowerCase());
					}
				}
				if (seedData.ic_compatibilities) {
					const seenIcs = new Set(this.data.ic_compatibilities.map((ic) => `${ic.ic_number}-${ic.component_type}-${ic.compatible_devices}`.toLowerCase()));
					let nextId = Date.now();
			...
			const seedPath = path.default.join(electron.app.getAppPath(), "default_seed.json");
			if (fs.default.existsSync(seedPath)) try {
				this.data = JSON.parse(fs.default.readFileSync(seedPath, "utf8"));
			} catch (e) {
	...
	save() {
		...
		try {
			fs.default.writeFileSync(tmpPath, JSON.stringify(this.data, null, 2));
			fs.default.renameSync(tmpPath, dbPath);
			return true;
		} catch (err) {
	...
	getCurrentMonth() {
		return this.data.months.find((m) => !m.is_closed) || this.data.months[this.data.months.length - 1];
	}
};
var db = new SimpleDB();
function initDB() {
...
function getBackupDir() {
	const userDataPath = process.env.TEST_USER_DATA || electron.app.getPath("userData");
	const dir = node_path.default.join(userDataPath, "backups_v2");
	if (!node_fs.default.existsSync(dir)) node_fs.default.mkdirSync(dir, { recursive: true });
	return dir;
}
function getCanonicalDatabaseHash(database) {
	if (!database) return node_crypto.default.createHash("sha256").update("", "utf8").digest("hex");
	const sortKeys = (obj) => {
		if (obj === null || typeof obj !== "object") return obj;
		if (Array.isArray(obj)) return obj.map(sortKeys);
		const sortedObj = {};
		const keys = Object.keys(obj).sort();
		for (const key of keys) sortedObj[key] = sortKeys(obj[key]);
		return sortedObj;
	};
	const canonicalDb = sortKeys(database);
	const jsonStr = JSON.stringify(canonicalDb);
	return node_crypto.default.createHash("sha256").update(jsonStr, "utf8").digest("hex");
}
function validateSchema(data) {
	...
	const checkDuplicates = (arr) => {
		...
		for (const item of arr) {
			...
			if (item.id === void 0) return false;
	...
	for (const op of data.operations) {
		if (op.cost !== void 0 && !Number.isFinite(op.cost)) return false;
		if (op.price !== void 0 && !Number.isFinite(op.price)) return false;
		if (op.shop_profit !== void 0 && !Number.isFinite(op.shop_profit)) return false;
		if (op.tech_profit !== void 0 && !Number.isFinite(op.tech_profit)) return false;
	}
	return true;
}
function createBackup(dbData, isManual = false) {
	try {
		...
		if (!isManual) {
			if (node_fs.default.readdirSync(backupDir).find((f) => f.startsWith("AutoBackup_" + dateStr))) return {
				success: true,
				reason: "AUTO_BACKUP_SKIPPED_ALREADY_EXISTS"
			};
		}
		const filename = `${isManual ? "ManualBackup" : "AutoBackup"}_${timestamp}.json`;
		const filepath = node_path.default.join(backupDir, filename);
		...
		node_fs.default.writeFileSync(tmpFilepath, JSON.stringify(backupContent, null, 2), "utf8");
		const writtenRaw = node_fs.default.readFileSync(tmpFilepath, "utf8");
		const parsed = JSON.parse(writtenRaw);
		if (!parsed || !parsed.database || !validateSchema(parsed.database)) {
			node_fs.default.unlinkSync(tmpFilepath);
			return {
				success: false,
				reason: "BACKUP_INVALID_SCHEMA"
			};
		}
		node_fs.default.renameSync(tmpFilepath, filepath);
		const allAutoBackups = node_fs.default.readdirSync(backupDir).filter((f) => f.startsWith("AutoBackup_") && f.endsWith(".json")).map((f) => ({
			name: f,
			path: node_path.default.join(backupDir, f),
			time: node_fs.default.statSync(node_path.default.join(backupDir, f)).mtime.getTime()
		})).sort((a, b) => b.time - a.time);
		if (allAutoBackups.length > MAX_BACKUPS) {
			const toDelete = allAutoBackups.slice(MAX_BACKUPS);
			for (const old of toDelete) try {
				node_fs.default.unlinkSync(old.path);
			} catch (e) {}
		}
		return {
			success: true,
...
function listBackups() {
	try {
		const backupDir = getBackupDir();
		const files = node_fs.default.readdirSync(backupDir).filter((f) => f.endsWith(".json"));
		const results = [];
		for (const file of files) try {
			const filepath = node_path.default.join(backupDir, file);
			const stats = node_fs.default.statSync(filepath);
			const content = JSON.parse(node_fs.default.readFileSync(filepath, "utf8"));
...
function readBackup(filename) {
	...
	if (filename.includes("/") || filename.includes("\\") || filename.includes("..")) throw new Error("BACKUP_INVALID_PATH");
	const safeName = node_path.default.basename(filename);
	if (!safeName.endsWith(".json")) throw new Error("BACKUP_INVALID_PATH");
	const backupDirResolved = node_path.default.resolve(getBackupDir());
	const filepath = node_path.default.resolve(backupDirResolved, safeName);
	if (!filepath.startsWith(backupDirResolved + node_path.default.sep)) throw new Error("BACKUP_INVALID_PATH");
	if (!node_fs.default.existsSync(filepath)) throw new Error("BACKUP_NOT_FOUND");
	...
	if (fileHash) {
		if (fileHash !== getCanonicalDatabaseHash(dbData)) throw new Error("BACKUP_HASH_MISMATCH");
	}
	return {
		data: dbData,
		fileHash
...
function createWindow() {
	...
	setInterval(() => createBackup(db.data, false), 36e5);
...
function generateMonthId() {
	...
	return Math.max(...db.data.months.map((m) => m.id)) + 1;
}
function setupIPC() {
	...
	electron.ipcMain.handle("update-settings", (_, settings) => {
	...
	electron.ipcMain.handle("get-technicians", () => {
		return db.data.technicians.filter((t) => t.is_active !== false);
	});
	electron.ipcMain.handle("add-technician", (_, name, profit_percentage) => {
		const newTech = {
			id: Date.now(),
			name,
			profit_percentage: Math.max(0, profit_percentage),
			is_active: true
		};
	...
	electron.ipcMain.handle("edit-technician", (_, id, name, profit_percentage) => {
		const tech = db.data.technicians.find((t) => t.id === id);
		if (tech) {
	...
	electron.ipcMain.handle("delete-technician", (_, id) => {
		const tech = db.data.technicians.find((t) => t.id === id);
		if (tech) {
	...
	electron.ipcMain.handle("get-operations", () => {
		return db.data.operations.filter((op) => op.month_id === getCurrentMonth().id).map((op) => {
			const tech = db.data.technicians.find((t) => t.id === op.technician_id);
			return {
				...op,
				technician_name: tech ? tech.name : "Unknown"
			};
		}).reverse();
	});
	electron.ipcMain.handle("get-all-operations", () => {
		return db.data.operations.map((op) => {
			const tech = db.data.technicians.find((t) => t.id === op.technician_id);
			return {
				...op,
				technician_name: tech ? tech.name : "Unknown"
	...
	electron.ipcMain.handle("add-operation", (_, op) => {
		const newId = Date.now();
		if (!db.data.technicians.find((t) => t.id === op.technician_id)) return {
			success: false,
			reason: "UNKNOWN_TECHNICIAN"
		};
		...
		if (![
			...
			"completed",
			"delivered"
		].includes(op.status)) op.status = "under_maintenance";
		if (!["cash", "debt"].includes(op.payment_status)) op.payment_status = "cash";
		const newOp = {
			id: newId,
	...
	electron.ipcMain.handle("edit-operation", (_, opId, updatedOp) => {
		const idx = db.data.operations.findIndex((o) => o.id === opId);
		if (idx !== -1) {
			if (db.data.operations[idx].month_id !== getCurrentMonth().id) return {
				success: false,
				reason: "CANNOT_EDIT_PAST_MONTH"
			};
			if (updatedOp.technician_id) {
				if (!db.data.technicians.find((t) => t.id === updatedOp.technician_id)) return {
					success: false,
					reason: "UNKNOWN_TECHNICIAN"
				};
			}
			...
			if (updatedOp.status && ![
				...
				"completed",
				"delivered"
			].includes(updatedOp.status)) updatedOp.status = "under_maintenance";
			if (updatedOp.payment_status && !["cash", "debt"].includes(updatedOp.payment_status)) updatedOp.payment_status = "cash";
	...
	electron.ipcMain.handle("delete-operation", (_, opId) => {
		const idx = db.data.operations.findIndex((o) => o.id === opId);
		if (idx !== -1) {
			if (db.data.operations[idx].month_id !== getCurrentMonth().id) return {
				success: false,
				reason: "CANNOT_DELETE_PAST_MONTH"
			};
	...
	electron.ipcMain.handle("get-debts", () => {
		return db.data.operations.filter((op) => op.payment_status === "debt").map((op) => {
			const tech = db.data.technicians.find((t) => t.id === op.technician_id);
			return {
				...op,
				technician_name: tech ? tech.name : "Unknown"
	...
	electron.ipcMain.handle("pay-debt", (_, operation_id) => {
		const op = db.data.operations.find((o) => o.id === operation_id);
		if (op) {
			if (op.payment_status !== "debt") return {
				success: false,
				reason: "DEBT_ALREADY_PAID"
			};
			...
			op.paid_at = (/* @__PURE__ */ new Date()).toISOString();
			if (!db.save()) {
	...
	electron.ipcMain.handle("get-withdrawals", () => {
		return db.data.withdrawals.map((w) => {
			const tech = db.data.technicians.find((t) => t.id === w.technician_id);
			return {
				...w,
	...
	electron.ipcMain.handle("add-withdrawal", (_, w) => {
		const newId = Date.now();
		if (typeof w.amount !== "number" || !Number.isFinite(w.amount)) w.amount = Number(w.amount) || 0;
		if (w.amount < 0) w.amount = 0;
		if (!["shop_withdrawal", "tech_withdrawal"].includes(w.type)) w.type = "shop_withdrawal";
		if (w.type === "tech_withdrawal") {
			if (!db.data.technicians.find((t) => t.id === w.technician_id)) return {
				success: false,
				reason: "UNKNOWN_TECHNICIAN"
			};
		}
	...
	electron.ipcMain.handle("edit-withdrawal", (_, id, updatedW) => {
		const idx = db.data.withdrawals.findIndex((w) => w.id === id);
		if (idx !== -1) {
			if (db.data.withdrawals[idx].month_id !== getCurrentMonth().id) return {
				success: false,
				reason: "CANNOT_EDIT_PAST_MONTH"
			};
			if (typeof updatedW.amount !== "undefined") updatedW.amount = Math.max(0, Number(updatedW.amount) || 0);
			const oldW = { ...db.data.withdrawals[idx] };
			delete updatedW.id;
			delete updatedW.month_id;
			delete updatedW.date;
			db.data.withdrawals[idx] = {
				...oldW,
				...updatedW
			};
			if (!db.save()) {
	...
	electron.ipcMain.handle("delete-withdrawal", (_, id) => {
		const idx = db.data.withdrawals.findIndex((w) => w.id === id);
		if (idx !== -1) {
			if (db.data.withdrawals[idx].month_id !== getCurrentMonth().id) return {
				success: false,
	...
	electron.ipcMain.handle("get-dashboard-stats", () => {
		...
		const currentMonthOps = db.data.operations.filter((op) => op.month_id === currentMonth.id);
		const deliveredOps = currentMonthOps.filter((op) => op.status === "delivered");
		const totalProfit = deliveredOps.reduce((sum, op) => sum + ((op.price || 0) - (op.cost || 0)), 0);
		const cashOps = deliveredOps.filter((op) => op.payment_status === "cash" && !op.paid_in_month_id);
		const paidDebts = db.data.operations.filter((op) => op.paid_in_month_id === currentMonth.id && op.status === "delivered");
		const unpaidDebts = deliveredOps.filter((op) => op.payment_status === "debt");
		const totalCashReceived = cashOps.reduce((sum, op) => sum + (op.price || 0), 0) + paidDebts.reduce((sum, op) => sum + (op.price || 0), 0);
		const totalOpsCost = currentMonthOps.reduce((sum, op) => sum + (op.cost || 0), 0);
		const currentMonthWithdrawals = db.data.withdrawals.filter((w) => w.month_id === currentMonth.id);
		const totalWithdrawals = currentMonthWithdrawals.reduce((sum, w) => sum + (w.amount || 0), 0);
		return {
			cashBox: currentMonth.start_capital + totalCashReceived - totalOpsCost - totalWithdrawals,
			totalProfit,
			debtTotal: unpaidDebts.reduce((sum, op) => sum + (op.price || 0), 0),
			totalWithdrawals,
			baseCapital: currentMonth.start_capital,
			tiedCapital: unpaidDebts.reduce((sum, op) => sum + (op.cost || 0), 0),
			availableCapital: currentMonth.start_capital - unpaidDebts.reduce((sum, op) => sum + (op.cost || 0), 0),
			realizedShopProfit: cashOps.reduce((sum, op) => sum + (op.shop_profit || 0), 0) + paidDebts.reduce((sum, op) => sum + (op.shop_profit || 0), 0),
			totalShopWithdrawal: currentMonthWithdrawals.filter((w) => w.type === "shop_withdrawal").reduce((sum, w) => sum + (w.amount || 0), 0),
			shopDue: currentMonth.start_capital - unpaidDebts.reduce((sum, op) => sum + (op.cost || 0), 0) + deliveredOps.reduce((sum, op) => sum + (op.shop_profit || 0), 0)
		};
	});
	electron.ipcMain.handle("get-technician-stats", () => {
		const currentMonth = getCurrentMonth();
		return db.data.technicians.map((tech) => {
			const techOpsThisMonth = db.data.operations.filter((op) => op.technician_id === tech.id && op.month_id === currentMonth.id);
			const totalCost = techOpsThisMonth.reduce((sum, op) => sum + (op.cost || 0), 0);
			const cashOps = techOpsThisMonth.filter((op) => op.payment_status === "cash" && !op.paid_in_month_id);
			const paidDebts = db.data.operations.filter((op) => op.technician_id === tech.id && op.paid_in_month_id === currentMonth.id);
			const realizedProfit = cashOps.reduce((sum, op) => sum + (op.tech_profit || 0), 0) + paidDebts.reduce((sum, op) => sum + (op.tech_profit || 0), 0);
			const unrealizedProfit = db.data.operations.filter((op) => op.technician_id === tech.id && op.payment_status === "debt").reduce((sum, op) => sum + (op.tech_profit || 0), 0);
			const techWithdrawal = db.data.withdrawals.filter((w) => w.type === "tech_withdrawal" && w.technician_id === tech.id && w.month_id === currentMonth.id).reduce((sum, w) => sum + (w.amount || 0), 0);
			return {
				id: tech.id,
				name: tech.name,
	...
	electron.ipcMain.handle("get-ic-compatibilities", () => {
		return db.data.ic_compatibilities || [];
	});
	electron.ipcMain.handle("add-ic-compatibility", (_, ic) => {
		const newIc = {
			...ic,
			id: Date.now()
		};
	...
	electron.ipcMain.handle("edit-ic-compatibility", (_, id, ic) => {
		const idx = db.data.ic_compatibilities.findIndex((i) => i.id === id);
		if (idx !== -1) {
			const oldIc = { ...db.data.ic_compatibilities[idx] };
			db.data.ic_compatibilities[idx] = {
				...oldIc,
				...ic
			};
			if (!db.save()) {
	...
	electron.ipcMain.handle("delete-ic-compatibility", (_, id) => {
		const idx = db.data.ic_compatibilities.findIndex((i) => i.id === id);
		if (idx !== -1) {
	...
	electron.ipcMain.handle("import-operations-excel-data", async (_, data) => {
		try {
			let added = 0;
			let ignored = 0;
			const currentMonth = getCurrentMonth();
			...
			for (let i = 1; i < data.length; i++) {
				...
				const date = row[0] ? String(row[0]).trim() : (/* @__PURE__ */ new Date()).toLocaleDateString("en-GB");
				...
				let tech = db.data.technicians.find((t) => t.name === techName);
				if (!tech) {
					ignored++;
					continue;
				}
				let techId = tech.id;
				if (db.data.operations.some((op) => opId && op.id === opId || op.customer_name === customerName && op.device === device && op.date === date)) {
					ignored++;
					continue;
				}
				const safeId = opId && !db.data.operations.some((o) => o.id === opId) ? opId : currentTimestamp++;
	...
	electron.ipcMain.handle("import-operations-excel", async () => {
		...
		try {
			const workbook = readFileSync(filePaths[0]);
	...
	electron.ipcMain.handle("import-ic-excel-data", async (_, data) => {
		try {
			let added = 0;
			let updated = 0;
			let ignored = 0;
			...
			let maxId = db.data.ic_compatibilities.reduce((max, ic) => Math.max(max, ic.id), 0);
			for (let i = 1; i < data.length; i++) {
				const row = data[i];
				...
				if (!icCode) continue;
				const existingIdx = db.data.ic_compatibilities.findIndex((ic) => String(ic.ic_number || "").toLowerCase() === icCode.toLowerCase());
				if (existingIdx !== -1) {
					const existingDevices = String(db.data.ic_compatibilities[existingIdx].compatible_devices || "").split(/[,=]/).map((d) => d.trim()).filter(Boolean);
					const newDevices = String(devicesStr || "").split(/[,=]/).map((d) => d.trim()).filter(Boolean);
	...
	electron.ipcMain.handle("import-ic-excel", async () => {
		const { canceled, filePaths } = await electron.dialog.showOpenDialog({
	...
	electron.ipcMain.handle("close-month", (_, newCapital) => {
		if (typeof newCapital !== "number" || !Number.isFinite(newCapital) || Number.isNaN(newCapital) || newCapital < 0) return {
			success: false,
			reason: "INVALID_NEW_CAPITAL"
		};
		const currentMonth = getCurrentMonth();
		const oldIsClosed = currentMonth.is_closed;
		const oldClosedAt = currentMonth.closed_at;
		currentMonth.is_closed = true;
		currentMonth.closed_at = (/* @__PURE__ */ new Date()).toISOString();
		const newMonth = {
			id: generateMonthId(),
			month_name: (/* @__PURE__ */ new Date()).toLocaleDateString("ar-EG", {
				month: "long",
		...
		if (!db.save()) {
			...
			return {
				success: false,
				reason: "MONTH_CLOSE_SAVE_FAILED"
			};
		}
		return { success: true };
	});
	electron.ipcMain.handle("close-month-with-excel", async (_, newCapital) => {
		if (typeof newCapital !== "number" || !Number.isFinite(newCapital) || Number.isNaN(newCapital) || newCapital < 0) return {
			success: false,
			reason: "INVALID_NEW_CAPITAL"
		};
		const currentMonth = getCurrentMonth();
		const ops = db.data.operations.filter((op) => op.month_id === currentMonth.id || op.paid_in_month_id === currentMonth.id);
		const allDebts = db.data.operations.filter((op) => op.payment_status === "debt");
		const tiedCapital = allDebts.filter((op) => op.month_id === currentMonth.id).reduce((sum, op) => sum + (op.cost || 0), 0);
		const availableCapital = currentMonth.start_capital - tiedCapital;
		const cashOps = ops.filter((op) => op.payment_status === "cash" && !op.paid_in_month_id);
		const paidDebts = db.data.operations.filter((op) => op.paid_in_month_id === currentMonth.id);
		const realizedShopProfit = cashOps.reduce((sum, op) => sum + (op.shop_profit || 0), 0) + paidDebts.reduce((sum, op) => sum + (op.shop_profit || 0), 0);
		const shopWithdrawals = db.data.withdrawals.filter((w) => w.type === "shop_withdrawal" && w.month_id === currentMonth.id).reduce((sum, w) => sum + (w.amount || 0), 0);
		const totalTechWithdrawals = db.data.withdrawals.filter((w) => w.type === "tech_withdrawal" && w.month_id === currentMonth.id).reduce((sum, w) => sum + (w.amount || 0), 0);
		const totalTechProfit = ops.reduce((sum, op) => sum + (op.tech_profit || 0), 0);
		const debtTotal = allDebts.reduce((sum, op) => sum + (op.price || 0), 0);
		const summaryData = [
		...
		const wsOps = utils.json_to_sheet(opsLogData);
		...
		utils.book_append_sheet(wb, wsSummary, "الخلاصة");
		utils.book_append_sheet(wb, wsOps, "سجل العمليات");
		...
		try {
			...
			node_fs.default.writeFileSync(filePath, buf);
		} catch (err) {
			return {
				success: false,
				reason: "error",
	...
	electron.ipcMain.handle("get-scrap-devices", () => {
		return db.data.scrap_devices || [];
	});
	electron.ipcMain.handle("add-scrap-device", (_, data) => {
		if (!db.data.scrap_devices) db.data.scrap_devices = [];
		const newId = Date.now();
		db.data.scrap_devices.push({
			id: newId,
			...data
		});
		if (!db.save()) {
	...
	electron.ipcMain.handle("edit-scrap-device", (_, id, data) => {
		const idx = db.data.scrap_devices.findIndex((d) => d.id === id);
		if (idx !== -1) {
			const oldD = { ...db.data.scrap_devices[idx] };
			db.data.scrap_devices[idx] = {
				...oldD,
				...data
			};
			if (!db.save()) {
	...
	electron.ipcMain.handle("delete-scrap-device", (_, id) => {
		const idx = db.data.scrap_devices.findIndex((d) => d.id === id);
		if (idx !== -1) {
			...
			if (!db.save()) {
				...
				return {
					success: false,
					reason: "SCRAP_DEVICE_DELETE_SAVE_FAILED"
				};
			}
			return { success: true };
		}
	...
	electron.ipcMain.handle("get-quick-lists", () => {
		return {
			devices: db.data.common_devices || [],
			faults: db.data.common_faults || []
		};
	});
	electron.ipcMain.handle("add-quick-list-item", (_, type, item) => {
	...
	electron.ipcMain.handle("remove-quick-list-item", (_, type, item) => {
		const targetArray = type === "device" ? "common_devices" : "common_faults";
		if (db.data[targetArray]) {
			const oldArray = [...db.data[targetArray]];
			db.data[targetArray] = db.data[targetArray].filter((i) => i !== item);
			if (!db.save()) {
	...
	electron.ipcMain.handle("create-full-backup", async () => {
		try {
			const wb = utils.book_new();
			const wsOps = utils.json_to_sheet(db.data.operations || []);
			utils.book_append_sheet(wb, wsOps, "العمليات");
			const wsWith = utils.json_to_sheet(db.data.withdrawals || []);
			utils.book_append_sheet(wb, wsWith, "السحوبات");
			const wsTech = utils.json_to_sheet(db.data.technicians || []);
			utils.book_append_sheet(wb, wsTech, "الفنيين");
			const defaultPath = `Full_Backup_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.xlsx`;
			const { canceled, filePath } = await electron.dialog.showSaveDialog({
			...
			node_fs.default.writeFileSync(jsonFilePath, JSON.stringify(db.data, null, 2));
			return { success: true };
		} catch (err) {
			return {
				success: false,
	...
	electron.ipcMain.handle("backup:restore", (_, filename) => {
		try {
			...
			try {
				...
				return {
					success: false,
					reason: err.message,
					message: "الملف غير صالح أو معطوب."
				};
			}
			...
			if (!preRestoreBackup.success) return {
				success: false,
				reason: "CURRENT_BACKUP_FAILED",
				message: "تعذر إنشاء نسخة احتياطية من البيانات الحالية، لذلك لم يتم تنفيذ الاستعادة."
			};
			db.data = restoredData;
			if (!db.save()) {
				db.load();
				if (getCanonicalDatabaseHash(db.data) !== originalHash) return {
					success: false,
					reason: "RESTORE_ROLLBACK_SAVE_FAILED",
					message: "CRITICAL RECOVERY ERROR"
				};
				return {
					success: false,
					reason: "DATABASE_SAVE_FAILED",
					message: "فشل حفظ قاعدة البيانات إلى القرص."
				};
			}
			db.load();
			...
			if (actualHash !== expectedHash || !opsMatch || !monthsMatch || !techsMatch || !withsMatch) {
				try {
					db.data = readBackup(node_path.default.basename(preRestoreBackup.filename)).data;
					const rbSave = db.save();
					db.load();
					const rollbackHash = getCanonicalDatabaseHash(db.data);
					if (!rbSave || rollbackHash !== originalHash) return {
						success: false,
				...
				return {
					success: false,
					reason: "RESTORE_VERIFY_FAILED",
					message: "فشلت عملية الاستعادة. تم التراجع بنجاح."
				};
			}
			return { success: true };
		} catch (e) {
	...
	electron.ipcMain.handle("factory-reset", () => {
		const originalHash = getCanonicalDatabaseHash(db.data);
		const backupResult = createBackup(db.data, true);
		if (!backupResult.success) return {
			success: false,
			reason: "FACTORY_RESET_BACKUP_FAILED",
			message: "فشل إنشاء نسخة احتياطية إجبارية. تم إيقاف عملية التصفير لحماية البيانات."
		};
		db.data.operations = [];
		db.data.withdrawals = [];
		if (db.data.technicians) db.data.technicians.forEach((t) => {
			t.start_balance = 0;
		});
		db.data.months = [{
			id: 1,
		...
		if (!db.save()) {
			try {
				db.data = readBackup(node_path.default.basename(backupResult.filename)).data;
				if (!db.save()) return {
					success: false,
					reason: "FACTORY_RESET_ROLLBACK_SAVE_FAILED",
					message: "CRITICAL RECOVERY ERROR"
				};
				db.load();
				if (getCanonicalDatabaseHash(db.data) !== originalHash) return {
					success: false,
					reason: "FACTORY_RESET_ROLLBACK_FAILED",
					message: "CRITICAL RECOVERY ERROR"
				};
			} catch (e) {
				return {
			...
			return {
				success: false,
				reason: "FACTORY_RESET_FAILED",
				message: "فشل التصفير أثناء الحفظ. تم الاحتفاظ بالبيانات."
			};
		}
		db.load();
		if (db.data.operations.length !== 0 || db.data.months.length !== 1) {
			...
			return {
				success: false,
				reason: "FACTORY_RESET_FAILED",
				message: "فشل التحقق من التصفير. تم التراجع."
			};
		}
		return { success: true };
	});
